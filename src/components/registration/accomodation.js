...
const Accomodation = ({
  ...
  const fillTestData = () => {
    ...
    setValue(
      ...
      availableRegistrationTypes.length > 0 ? String(availableRegistrationTypes[0].id) : ""
    );
  ...
  return (
    <>
      ...
      <div className={classNames(cssForm.smallW, "mx-auto position-relative")}>
          ...
          <div className="mb-4 mt-2">
                  ...
                  <div key={registration.id} className="form-check">
                    <input
                      ...
                      className={classNames("form-check-input", {
                        "is-invalid": !!errors.registration_type_id,
                      })}
                      value={registration.id}
                      {...register("registration_type_id", {
                        required: "Please select a registration type",
                      })}
                    />
                    <label
                      className="form-check-label d-block"
                      htmlFor={`registration-${registration.id}`}
                    >
                      <strong>
                        {isEarlyBird
                          ? parseFloat(registration.price).toFixed(2)
                          : (
                            parseFloat(registration.price) +
                            conferenceData.costs.after_early_birds
                          ).toFixed(2)}
                        €
                      </strong>
                      {" - "}
            ...
            {errors.registration_type_id && (
              <p className="text-danger">
                <small>{errors.registration_type_id.message}</small>
              </p>
            )}
          </div>
        )}
        ...
        {isOnline && (
          <p>
            The price for online participation in IMC {conferenceData.year} is{" "}
            <b className="fw-bolder">{conferenceData.costs.online}€</b>.
          </p>
        )}
        ...
        <div className="mb-3">
          ...
          <div className="d-flex flex-column gap-2">
            {paymentMethods.length > 0 ? (
              paymentMethods.map((method) => (
                <div key={method.id} className="form-check">
                  <input
                    ...
                    className={classNames("form-check-input", {
                      "is-invalid": errors.payment_method_id,
                    })}
                    value={method.id}
              ...
              <p className="text-danger">
                <small>No payment methods available.</small>
              </p>
            )}
          </div>
          ...
          {errors.payment_method_id && (
            <p className="text-danger">
              <small>{errors.payment_method_id.message}</small>
            </p>
          )}
        </div>
      </div>
